...
const helperNames = ['$if', ...Object.keys(extendedFunctions).filter((n) => n.startsWith('$'))];
const functions = helperNames.slice(1).map((name) => {
	const fn = extendedFunctions[name];
	const doc = (fn as { doc?: Doc }).doc;
...
${helperNames.map((n) => `\t\t${n}: typeof ${n};`).join('\n')}
	}
}
